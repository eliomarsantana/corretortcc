...
public class ProcessFile extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * flb.createText(); Abstract r = flb.createAbstract(TEXTO_COMPLETO);
		 * Title t = flb.createTitle(TEXTO_COMPLETO); Section s =
		 * flb.createSection(TEXTO_COMPLETO); text.setResumo(r);
		 * text.setTitulo(t); text.setSection(s); //text.mountText();
		 * 
		 * 
		 * 
		 * 
		 ...
		 * 
		 * String sections2 =
		 * util.retiraCaracterEspecial(util.UTF8toISO(sections)); String
		 * inputSection2 = ""; Matcher matcher2 = pattern.matcher(sections2);
		 * 
		 * while (matcher2.find()) {
		 * 
		 ...
		 * response);
		 * 
		 * } else { request.setAttribute("erro",
		 * "Somente upload de arquivo .tex");
